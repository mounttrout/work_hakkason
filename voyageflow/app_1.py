...
from ui.ui_plan import render_chat_history, render_planning_summary, render_timeline_visibility_controls
from ui.ui_execution import render_itinerary_cards, status_emoji
...
with tabs[0]:
    ...
    with left:
        ...
        st.divider()
        render_chat_history(st.session_state.chat_history, conversation_advisor_questions, append_chat)
    ...
    with right:
        render_planning_summary(st.session_state.planning_state, st.session_state.get("resolved_conditions", {}))
...
with tabs[2]:
    ...
    if st.session_state.df_phase3 is not None:
        ...
        render_itinerary_cards(
            df=df_phase3,
            safe_text_fn=safe_text,
            build_google_maps_search_url_fn=build_google_maps_search_url,
            build_google_maps_dir_url_fn=build_google_maps_dir_url,
            build_transport_display_fn=build_transport_display,
            clean_address_fn=clean_address,
            format_genre_fn=format_genre,
            format_purpose_fn=format_purpose,
            get_rental_car_availability_fn=get_rental_car_availability,
            apply_change_fn=lambda scope, idx, mode: apply_transport_change_to_plan(idx, mode) if scope == "plan" else apply_transport_change_during_execution(idx, mode),
        ...
        if not getattr(engine, "execution_started", False):
            ...
            if st.session_state.show_mood_dialog:
                ...
                if st.session_state.get("mood_flow_mode", "menu") != "consult":
                    ...
                    with mc1:
                        if st.button("🚶 寄り道", key="mood_detour", use_container_width=True):
                            try:
                                ...
                                if st.session_state.debug_mode:
                                    st.exception(e)
                    with mc2:
                        if st.button("⏭️ 次の予定をキャンセル", key="mood_cancel_next", use_container_width=True):
                            try:
                    ...
                    with mc3:
                        if st.button("🏨 その日の予定をキャンセルして宿泊先へ移動", key="mood_cancel_day_to_hotel", use_container_width=True):
                            try:
                                ...
                                if st.session_state.debug_mode:
                                    st.exception(e)
                    with mc4:
                        if st.button("🏠 予定を全てキャンセルして帰着地へ移動", key="mood_return_home", use_container_width=True):
                            try:
                    ...
                    with mc5:
                        if st.button("🚕 移動方法の変更", key="mood_transport_change", use_container_width=True):
                            try:
                                ...
                                if st.session_state.debug_mode:
                                    st.exception(e)
                    with mc6:
                        if st.button("💬 自由会話による相談とボタンを出してその先の処理を変えるように変更したい", key="mood_consult_mode", use_container_width=True):
                    ...
                    with md1:
                        if st.button("この内容で組み直し案を作る", key="mood_consult_apply", use_container_width=True):
                            try:
                                ...
                                if st.session_state.debug_mode:
                                    st.exception(e)
                    with md2:
                        if st.button("一覧に戻る", key="mood_consult_back", use_container_width=True):
                            st.session_state.mood_flow_mode = "menu"
                            st.rerun()
