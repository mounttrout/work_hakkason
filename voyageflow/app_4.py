        ...
        with st.expander(f"Day {int(day)} - {date_label}{suffix}", expanded=(int(day) == 1)):
            ...
            for local_pos, (_, row) in enumerate(day_df.iterrows()):
                ...
                if is_transport:
                    ...
                    route_source = safe_text(row_dict.get("route_data_source"), "")
                    route_departure_at = safe_text(row_dict.get("route_departure_at"), "")
                    route_debug_reason = safe_text(row_dict.get("route_debug_reason"), "")
                    if route_source == "google_routes_api":
                        route_data_text = f"移動時間: 実検索（Google Routes / {route_departure_at or '日時不明'} 出発想定）"
                    elif route_source == "fallback_missing_coordinates":
                        route_data_text = "移動時間: 推定値（フォールバック / 座標未取得）"
                    elif route_source == "fallback_routes_unavailable":
                        route_data_text = "移動時間: 推定値（フォールバック / Routes API未取得）"
                    else:
                        route_data_text = "移動時間: 推定値（フォールバック）"
                    debug_block = ""
                    if route_debug_reason not in {"", "-"}:
                        debug_block = f"<div style='margin-top:4px;color:#8a5a00;'>補足: {html.escape(route_debug_reason)}</div>"
                    body = f"""
<div class="vf-card {card_class}">
  ...
  <div>{html.escape(route_data_text)}</div>
  {debug_block}
</div>
"""
                    ...
                    if allow_transport_edit and absolute_idx is not None:
                        with st.expander(f"移動手段を変更 Day{int(day)}-Step{absolute_idx + 1}", expanded=False):
                            ...
                            for col, (mode_key, mode_label_button) in zip(cols, mode_options.items()):
                                ...
                                if col.button(label, key=f"pick_{transport_edit_scope}_{day}_{absolute_idx}_{local_pos}_{mode_key}", width='stretch', disabled=disabled):
                            ...
                            if st.button("この移動手段に変更", key=f"apply_transport_{transport_edit_scope}_{day}_{absolute_idx}_{local_pos}", width='stretch', disabled=apply_disabled):
                                try:
...
"""
                    ...
                    if transport_edit_scope == "plan" and absolute_idx is not None:
                        ...
                        with st.expander("このスポットを編集", expanded=False):
                            ...
                            with edit_cols[0]:
                                if st.button("このスポットを削除", key=f"delete_spot_{absolute_idx}_{local_pos}", width='stretch'):
                                    try:
                            ...
                            if st.button("この内容でスポット変更", key=f"update_spot_{absolute_idx}_{local_pos}", width='stretch'):
                                try:
        ...
        if not getattr(engine, "execution_started", False):
            ...
            if st.session_state.show_mood_dialog:
                ...
                with md1:
                    ...
                    if st.button(button_label, key="mood_action_apply", width='stretch'):
                        try:
