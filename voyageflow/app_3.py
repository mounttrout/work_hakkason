...
<style>
    ...
    .vf-chat-user {
        ...
        margin-bottom: 8px; border: 1px solid #b9d6fb; line-height: 1.6; font-size: 16px;
    }
    .vf-chat-ai {
        padding: 12px 14px; border-radius: 12px; background: #f2f4f7; color: #1d2633;
        margin-bottom: 8px; border: 1px solid #d8dee8; line-height: 1.6; font-size: 16px;
    }
    .vf-tip { padding: 10px 12px; border-radius: 10px; background: #f8fff2; color: #19340d; border: 1px solid #d8efc2; }
    .vf-card {
        padding: 14px 16px; border-radius: 14px; margin-bottom: 10px; border: 1px solid #d8d8d8; line-height: 1.65;
    ...
    .vf-card-note {
        margin-top: 6px; margin-bottom: 8px; padding: 10px 12px; border-radius: 10px;
        background: rgba(255,255,255,0.80); color: #6b4308; border: 1px dashed #d49a43; line-height: 1.6;
    ...
    .vf-log-item {
        padding: 10px 12px; border-radius: 10px; margin-bottom: 8px; background: rgba(255,255,255,0.82);
        color: inherit; border: 1px solid rgba(120, 150, 190, 0.20); line-height: 1.6; word-break: break-word;
        font-size: 15px;
    }
    ...
    @media (prefers-color-scheme: dark) {
        .vf-chat-user { background: #1f3046; color: #eef5ff; border-color: #3d5a7d; }
        .vf-chat-ai { background: #1f2430; color: #f5f7fb; border-color: #3d4558; }
        .vf-tip { background: #1d2b1e; color: #effbe8; border-color: #3a5c3c; }
        .vf-card-completed { background: #2b3138; border-color: #49525c; color: #f3f4f6; }
        .vf-card-current { background: #173226; border-color: #2f8f65; color: #effcf5; }
        .vf-card-future { background: #12263a; border-color: #37648d; color: #eef6ff; }
        .vf-card-modified { background: #3d2a15; border-color: #df9b3f; color: #fff3de; }
        .vf-card-note { background: rgba(26, 29, 35, 0.88); color: #fff6dc; border-color: #d49a43; }
        .vf-log-panel { background: rgba(28, 35, 49, 0.92); border-color: rgba(111, 133, 166, 0.45); color: #eef3fb; }
        .vf-log-item { background: rgba(255,255,255,0.10); color: #f5f7fb; border-color: rgba(158, 178, 209, 0.28); }
    }
    @media (max-width: 768px) {
        ...
        .vf-card, .vf-chat-user, .vf-chat-ai { font-size: 15px; }
...
def init_session_state() -> None:
    defaults = {
        ...
        "replan_preview_scope": "all_future",
        "replan_preview_action": None,
        "replan_error": "",
...
def format_phase1_preview_text(plan_text: str) -> str:
    ...
    text = re.sub(
        r"^テーマ:\s*(.*)$",
        r"<div style='font-size:0.95rem;color:#6b7280;margin-bottom:12px;'>テーマ: \1</div>",
        text,
        flags=re.MULTILINE,
    )
    text = re.sub(
    ...
    text = re.sub(
        r"^\s*-\s*目的:\s*(.*)$",
        r"<div style='margin-left:16px;font-size:0.9rem;color:#374151;'>目的: \1</div>",
        text,
        flags=re.MULTILINE,
    )
    text = re.sub(
...
def render_internal_logs_sidebar() -> None:
    ...
    with c1:
        if st.button("🧹 ログをクリア", key="clear_app_logs", width="stretch"):
            clear_logs()
            st.rerun()
    with c2:
...
def reset_all() -> None:
    ...
    st.session_state.mood_flow_mode = "menu"
    st.session_state.mood_consultation_input = ""
    ...
    st.session_state.replan_preview_scope = "all_future"
    st.session_state.replan_preview_action = None
...
def generate_execution_replan_preview(change_request: str, source_event: str = "execution") -> None:
    ...
    # 直近の移動手段変更は LLM を使わず、差分プレビューだけを作る
    if prepare_direct_transport_change_preview(change_request, source_event=source_event):
        log_event("再計画", f"自由入力を直接差分として解釈: {change_request}")
        return

    scope = infer_replan_scope(change_request)
    remaining_plan_text = engine.build_replan_scope_text(scope)
    ...
    if scope in {"rest_of_day", "local"}:
        replanning_state["trip_days"] = 1
    else:
        replanning_state["trip_days"] = max(1, int(current_df["day"].max()) - int(current_row.get("day", 1)) + 1)

    scope_label = {"local": "直近の予定のみ", "rest_of_day": "その日の残りのみ", "all_future": "これから先すべて"}[scope]
...
【最重要】
- すでに終わった予定は変更しません。
- 今回は「{scope_label}」だけを見直してください。
- ユーザーが明示的に広域見直しを依頼していない限り、変更範囲を広げないでください。
- 以下に示す対象範囲だけを自然な日本語で組み直してください。
...
- 組み直し対象日数の目安: {replanning_state['trip_days']}日
...
""".strip()
    ...
    log_event("再計画", f"自由組み直し案を生成: scope={scope} / {revision_text}")
    ...
    st.session_state.replan_preview_scope = scope
    st.session_state.replan_preview_action = None
    st.session_state.replan_error = ""
...
def apply_execution_replan_preview() -> None:
    ...
    direct_action = st.session_state.get("replan_preview_action")
    if direct_action:
        if direct_action.get("type") == "transport_change":
            result = engine.update_transport_step(
                int(direct_action["step_index"]),
                str(direct_action["new_mode"]),
                reason=str(direct_action.get("reason") or "自由記述に基づく移動手段変更"),
            )
            ...
            st.session_state.replan_preview_scope = "all_future"
            st.session_state.replan_preview_action = None
            log_event("再計画", result.get("message", "移動手段変更を反映"))
            return
    ...
    engine.replace_future_plan(
        df3_new,
        reason=st.session_state.get("replan_preview_request", "自由組み直しを反映"),
        scope=st.session_state.get("replan_preview_scope", "all_future"),
    )
    st.session_state.df_phase3 = engine.get_updated_dataframe()
...
def _get_phase2_activity_indices(df_phase2: pd.DataFrame) -> List[int]:
    if df_phase2 is None or df_phase2.empty:
        return []
    if "is_transport" in df_phase2.columns:
        activity_df = df_phase2[df_phase2["is_transport"] == False]  # noqa: E712
    else:
        activity_df = df_phase2
    return activity_df.index.tolist()


def _get_phase2_activity_index_from_phase3_index(phase3_df: pd.DataFrame, phase3_index: int) -> Optional[int]:
    df_phase2 = st.session_state.get("df_phase2")
    if df_phase2 is None or df_phase2.empty or phase3_df is None or phase3_df.empty:
        return None
    if phase3_index is None or phase3_index < 0 or phase3_index >= len(phase3_df):
        return None
    row = phase3_df.iloc[phase3_index]
    if bool(row.get("is_transport", False)):
        return None

    activity_positions = phase3_df.index[phase3_df["is_transport"] == False].tolist()  # noqa: E712
    if phase3_index not in activity_positions:
        return None
    activity_order = activity_positions.index(phase3_index)

    phase2_activity_indices = _get_phase2_activity_indices(df_phase2)
    if activity_order >= len(phase2_activity_indices):
        return None
    return int(phase2_activity_indices[activity_order])


def _is_special_activity_row(df_phase2: pd.DataFrame, phase2_index: int) -> bool:
    if df_phase2 is None or df_phase2.empty or phase2_index is None or phase2_index not in df_phase2.index:
        return True
    activity_indices = _get_phase2_activity_indices(df_phase2)
    if not activity_indices:
        return True
    if phase2_index == activity_indices[0] or phase2_index == activity_indices[-1]:
        return True
    row = df_phase2.loc[phase2_index]
    destination = safe_text(row.get("destination"), "")
    purpose = safe_text(row.get("purpose"), "")
    genre = safe_text(row.get("genre"), "")
    combined = f"{destination} {purpose} {genre}".lower()
    return any(token in combined for token in ["ホテル", "hotel", "宿", "lodging"])


def rebuild_final_itinerary_from_phase2(updated_df2: pd.DataFrame, reason: str = "完成旅程を更新") -> None:
    s = st.session_state.planning_state
    df2 = normalize_phase2_dataframe(updated_df2.copy(), s)
    router = Phase3Routing(logger=log_event)
    df3 = router.insert_routes(df2, user_request=build_phase1_request_text(), transport_preference=s["transport_style"])
    if df3 is None or df3.empty:
...
def remove_activity_from_final_itinerary(phase3_index: int) -> str:
    df_phase2 = st.session_state.get("df_phase2")
    df_phase3 = st.session_state.get("df_phase3")
    if df_phase2 is None or df_phase2.empty or df_phase3 is None or df_phase3.empty:
        raise ValueError("編集対象の旅程データがありません。")

    phase2_index = _get_phase2_activity_index_from_phase3_index(df_phase3, phase3_index)
    if phase2_index is None:
        raise ValueError("このカードは削除できません。")
    if _is_special_activity_row(df_phase2, phase2_index):
        raise ValueError("出発地・帰着地・宿泊先にあたるカードは最小版では削除できません。")

    removed_name = safe_text(df_phase2.loc[phase2_index].get("destination"), "スポット")
    updated_df2 = df_phase2.drop(index=phase2_index).reset_index(drop=True)
    rebuild_final_itinerary_from_phase2(updated_df2, reason=f"{removed_name} を削除して完成旅程を再構築")
    return f"{removed_name} を削除し、前後の移動を含めて旅程を更新しました。"


def replace_activity_destination_in_final_itinerary(phase3_index: int, new_destination: str) -> str:
    df_phase2 = st.session_state.get("df_phase2")
    df_phase3 = st.session_state.get("df_phase3")
    if df_phase2 is None or df_phase2.empty or df_phase3 is None or df_phase3.empty:
        raise ValueError("編集対象の旅程データがありません。")
    ...
    phase2_index = _get_phase2_activity_index_from_phase3_index(df_phase3, phase3_index)
    if phase2_index is None:
        raise ValueError("このカードは変更できません。")
    if _is_special_activity_row(df_phase2, phase2_index):
        raise ValueError("出発地・帰着地・宿泊先にあたるカードは最小版では変更できません。")

    new_destination = safe_text(new_destination, "").strip()
    if not new_destination or new_destination == "-":
        raise ValueError("新しいスポット名を入力してください。")

    old_name = safe_text(df_phase2.loc[phase2_index].get("destination"), "スポット")
    updated_df2 = df_phase2.copy()
    updated_df2.at[phase2_index, "destination"] = new_destination
    if "address" in updated_df2.columns:
        updated_df2.at[phase2_index, "address"] = ""
    if "one_point" in updated_df2.columns:
        updated_df2.at[phase2_index, "one_point"] = f"ユーザーが完成旅程上でスポットを『{new_destination}』へ変更しました。"

    rebuild_final_itinerary_from_phase2(updated_df2, reason=f"{old_name} を {new_destination} に変更して完成旅程を再構築")
    return f"{old_name} を {new_destination} に変更し、前後の移動を含めて旅程を更新しました。"


def _render_final_plan_activity_editor(phase3_df: pd.DataFrame, phase3_index: int, row_dict: Dict) -> None:
    df_phase2 = st.session_state.get("df_phase2")
    phase2_index = _get_phase2_activity_index_from_phase3_index(phase3_df, phase3_index)
    if phase2_index is None or _is_special_activity_row(df_phase2, phase2_index):
        return

    editor_key = f"final_plan_editor_{phase3_index}"
    with st.expander("✏️ このカードを編集", expanded=False):
        st.caption("最小版: スポット削除 / スポット名変更に対応。変更後は前後の移動も再計算します。")
        action_cols = st.columns([1, 1])
        with action_cols[0]:
            if st.button("🗑️ このスポットを削除", key=f"delete_final_plan_{phase3_index}", width="stretch"):
                try:
                    message = remove_activity_from_final_itinerary(phase3_index)
                    st.success(message)
                    st.rerun()
                except Exception as e:
                    st.error(f"削除エラー: {e}")

        default_destination = safe_text(row_dict.get("destination"), "")
        new_destination = st.text_input(
            "スポット名を変更",
            value=default_destination,
            key=f"{editor_key}_destination",
            placeholder="例: 上野公園",
        )
        with action_cols[1]:
            if st.button("🔄 スポットを変更", key=f"replace_final_plan_{phase3_index}", width="stretch"):
                try:
                    message = replace_activity_destination_in_final_itinerary(phase3_index, new_destination)
                    st.success(message)
                    st.rerun()
                except Exception as e:
                    st.error(f"変更エラー: {e}")
...
def infer_replan_scope(user_text: str) -> str:
    text = str(user_text or "").strip()
    all_future_keywords = ["これから全部", "残り全部", "今後全部", "全行程", "全部見直", "全部組み直", "旅程を全部"]
    rest_of_day_keywords = ["今日の残り", "その日の残り", "今日これから", "今日の予定を", "今日の後半", "本日の残り"]
    if any(k in text for k in all_future_keywords):
        return "all_future"
    if any(k in text for k in rest_of_day_keywords):
        return "rest_of_day"
    return "local"


def _detect_transport_mode_from_text(text: str) -> Optional[str]:
    ...
    for keyword, mode in mapping:
        if keyword in normalized:
            return mode
    return None


def _find_transport_step_from_request(engine: ExecutionEngine, request_text: str) -> Optional[int]:
    mode = _detect_transport_mode_from_text(request_text)
    if not mode:
        return None
    df = engine.get_updated_dataframe()
    if df is None or df.empty:
        return None
    start_idx = int(getattr(engine, "current_step", 0))
    ...
    for idx in candidate_indices:
        destination_text = safe_text(df.iloc[idx].get("destination"), "")
        if "→" in destination_text:
            origin, destination = [part.strip() for part in destination_text.split("→", 1)]
        else:
            origin, destination = "", destination_text.strip()
        score = 0
        if origin and origin in text:
            score += 2
        if destination and destination in text:
            score += 2
        if origin and destination and f"{origin}から{destination}" in text:
            score += 3
        if origin and destination and f"{origin}→{destination}" in text:
            score += 3
        if idx == engine._find_next_transport_index():
            score += 1
        if score > best_score:
            best_score = score
            best_idx = idx
    return best_idx if best_score > 0 else engine._find_next_transport_index()


def prepare_direct_transport_change_preview(change_request: str, source_event: str = "execution") -> bool:
    engine = st.session_state.execution_engine
    if engine is None:
        return False
    new_mode = _detect_transport_mode_from_text(change_request)
    if not new_mode:
        return False
    step_index = _find_transport_step_from_request(engine, change_request)
    if step_index is None:
        return False

    df = engine.get_updated_dataframe()
    row = df.iloc[step_index]
    destination_text = safe_text(row.get("destination"), "")
    if "→" in destination_text:
        origin_name, destination_name = [part.strip() for part in destination_text.split("→", 1)]
    else:
        origin_name, destination_name = "現在地", destination_text
    old_mode = safe_text(row.get("transport_mode"), "walk").lower()
    duration_before = int(row.get("duration_minutes") or 0)
    duration_after = engine._estimate_transport_duration(step_index, new_mode, fallback_minutes=max(1, duration_before))
    mode_label_before = engine.TRANSPORT_LABELS.get(old_mode, old_mode)
    mode_label_after = engine.TRANSPORT_LABELS.get(new_mode, new_mode)

    preview_text = f"""今回の依頼は、広域な再計画ではなく直近の移動手段変更として解釈しました。

対象区間: {origin_name} → {destination_name}
変更前: {mode_label_before} / {duration_before}分
変更後: {mode_label_after} / {duration_after}分（目安）

この差分だけを反映します。
※移動時間や所要時間は目安です。完成旅程では、実際の移動経路や実時間にあわせて調整して表示します。"""

    st.session_state.replan_preview_draft = preview_text
    st.session_state.replan_preview_request = change_request.strip()
    st.session_state.replan_preview_source = source_event
    st.session_state.replan_preview_scope = "local"
    st.session_state.replan_preview_action = {
        "type": "transport_change",
        "step_index": int(step_index),
        "new_mode": new_mode,
        "reason": change_request.strip() or "自由記述に基づく移動手段変更",
    }
    st.session_state.replan_error = ""
    return True
...
def render_itinerary_cards(
    ...
    allow_activity_edit: bool = False,
) -> None:
    if df is None or df.empty:
        st.info("旅程データがありません。")
        return
...
"""
                    ...
                    st.link_button("🗺️ Google Mapsでルートを見る", route_url, width="stretch")
...
"""
                    ...
                    if note:
                        st.markdown(f"<div class='vf-card-note'>差分: {note}</div>", unsafe_allow_html=True)
                    st.link_button("📍 Google Mapsで場所を見る", place_url, width="stretch")
                    if allow_activity_edit and transport_edit_scope == "plan" and absolute_idx is not None:
                        _render_final_plan_activity_editor(df, absolute_idx, row_dict)
...
with st.sidebar:
    ...
    if st.button("🔄 全リセット", width="stretch"):
        reset_all()
        st.rerun()
...
with tabs[0]:
    ...
    with left:
        ...
        b1, b2 = st.columns(2)
        with b1:
            if st.button("🪄 旅行案を作成", width="stretch"):
                try:
        ...
        with b2:
            if st.button("🔁 旅行案を再作成", width="stretch"):
                try:
                    generate_phase1_draft()
...
with tabs[1]:
    ...
    if st.session_state.trip_plan_draft:
        ...
        with c1:
            if st.button("💬 修正を追加して再提案", width="stretch"):
        ...
        with c2:
            if st.button("✅ この案で了承する", width="stretch"):
                try:
        ...
        with c3:
            if st.button("🗑️ 下書きを破棄", width="stretch"):
                st.session_state.trip_plan_draft = None
                st.session_state.trip_plan = None
                st.session_state.df_phase2 = None
                st.session_state.df_phase3 = None
...
with tabs[2]:
    ...
    if st.session_state.df_phase3 is not None:
        ...
        with st.expander("Phase2 の構造化データ", expanded=False):
            ...
            st.dataframe(st.session_state.df_phase2[display_cols], width="stretch", height=320)
...
with tabs[3]:
    ...
    if st.session_state.df_phase3 is not None:
        ...
        if not getattr(engine, "execution_started", False):
            ...
            if st.button("🚀 旅程実行を開始", width="stretch"):
                try:
            ...
            with cbtn1:
                if st.button("✅ 予定通り進む", width="stretch"):
                    try:
            ...
            with cbtn2:
                if st.button("⏰ 遅延", width="stretch"):
                    st.session_state.show_delay_dialog = True

            with cbtn3:
                if st.button("🌥️ 天候不順", width="stretch"):
                    st.session_state.show_weather_dialog = True

            with cbtn4:
                if st.button("💭 気分が変わった", width="stretch"):
                    st.session_state.show_mood_dialog = True

            with cbtn5:
                if st.button("✈️ 欠航・運休 / 中止", width="stretch"):
            ...
            if st.session_state.show_delay_dialog:
                ...
                with dy1:
                    if st.button("簡易提案を出す", key="delay_simple", width="stretch"):
                        try:
                ...
                with dy2:
                    if st.button("自由に組み直し案を作る", key="delay_replan", width="stretch"):
                        try:
                ...
                with dy3:
                    if st.button("キャンセル", key="delay_cancel", width="stretch"):
                        st.session_state.show_delay_dialog = False
                        st.rerun()
            ...
            if st.session_state.show_weather_dialog:
                ...
                with wx1:
                    if st.button("簡易提案を出す", key="weather_simple", width="stretch"):
                        try:
                            ...
                            if st.session_state.debug_mode:
                                st.exception(e)
                with wx2:
                    if st.button("自由に組み直し案を作る", key="weather_replan", width="stretch"):
                        try:
                            ...
                            if st.session_state.debug_mode:
                                st.exception(e)
                with wx3:
                    if st.button("キャンセル", key="weather_cancel", width="stretch"):
                        st.session_state.show_weather_dialog = False
                        st.rerun()

            if st.session_state.show_mood_dialog:
                st.markdown("#### 💭 気分の変化")
                st.caption("やりたいことを選ぶと、その内容に応じた処理だけを出します。")

                if st.session_state.get("mood_flow_mode", "menu") != "consult":
                    mc1, mc2 = st.columns(2)
                    with mc1:
                        if st.button("🚶 寄り道", key="mood_detour", width="stretch"):
                            try:
                                generate_execution_replan_preview("少し寄り道したい。今の流れを大きく壊さず、立ち寄り先を1つ追加した組み直し案を作って。", source_event="mood_detour")
                                st.session_state.show_mood_dialog = False
                                st.session_state.mood_flow_mode = "menu"
                                st.rerun()
                            except Exception as e:
                                ...
                                if st.session_state.debug_mode:
                                    st.exception(e)
                    with mc2:
                        if st.button("⏭️ 次の予定をキャンセル", key="mood_cancel_next", width="stretch"):
                            try:
                                apply_result = engine.cancel_next_activity_only("気分が変わったため、次の予定のみキャンセル")
                                st.session_state.event_result = None
                                st.session_state.show_mood_dialog = False
                                st.session_state.mood_flow_mode = "menu"
                                st.success(apply_result.get("message", "次の予定をキャンセルしました。"))
                                st.rerun()
                            except Exception as e:
                                st.error(f"予定キャンセルエラー: {e}")
                                if st.session_state.debug_mode:
                                    st.exception(e)

                    mc3, mc4 = st.columns(2)
                    with mc3:
                        if st.button("🏨 その日の予定をキャンセルして宿泊先へ移動", key="mood_cancel_day_to_hotel", width="stretch"):
                            try:
                                apply_result = engine.cancel_today_and_move_to_lodging("気分が変わったため、本日の残り予定をキャンセルして宿泊先へ移動")
                                st.session_state.event_result = None
                                st.session_state.show_mood_dialog = False
                                st.session_state.mood_flow_mode = "menu"
                                st.success(apply_result.get("message", "本日の予定を宿泊先移動へ切り替えました。"))
                                st.rerun()
                            except Exception as e:
                                st.error(f"宿泊先移動エラー: {e}")
                                if st.session_state.debug_mode:
                                    st.exception(e)
                    with mc4:
                        if st.button("🏠 予定を全てキャンセルして帰着地へ移動", key="mood_return_home", width="stretch"):
                            try:
                                apply_result = engine.return_to_final_destination_now("気分が変わったため、残り予定をキャンセルして帰着地へ移動")
                                st.session_state.event_result = None
                                st.session_state.show_mood_dialog = False
                                st.session_state.mood_flow_mode = "menu"
                                st.success(apply_result.get("message", "残り予定をキャンセルして帰着地へ移動します。"))
                                st.rerun()
                            except Exception as e:
                                st.error(f"帰着地移動エラー: {e}")
                                if st.session_state.debug_mode:
                                    st.exception(e)

                    mc5, mc6 = st.columns(2)
                    with mc5:
                        if st.button("🚕 移動方法の変更", key="mood_transport_change", width="stretch"):
                            try:
                                st.session_state.event_result = engine.trigger_event("mood_change", "移動方法を変更したい。次の移動をもっと楽な方法にして。")
                                st.session_state.show_mood_dialog = False
                                st.session_state.mood_flow_mode = "menu"
                                st.rerun()
                            except Exception as e:
                                st.error(f"移動方法提案エラー: {e}")
                                if st.session_state.debug_mode:
                                    st.exception(e)
                    with mc6:
                        if st.button("💬 自由会話による相談とボタンを出してその先の処理を変えるように変更したい", key="mood_consult_mode", width="stretch"):
                            st.session_state.mood_flow_mode = "consult"
                            st.rerun()

                    if st.button("キャンセル", key="mood_cancel", width="stretch"):
                        st.session_state.show_mood_dialog = False
                        st.session_state.mood_flow_mode = "menu"
                        st.rerun()
                else:
                    consultation_value = st.text_area(
                        "相談内容を入力してください",
                        value=st.session_state.get("mood_consultation_input", ""),
                        placeholder="例: 少し疲れたので寄り道はやめたいけど、雰囲気の良いカフェには立ち寄りたい。残り旅程を軽めに組み直して。",
                        key="mood_consultation_input"
                    )
                    st.caption("この入力は、旧『自由に組みなおし案を作る』と同じ組み直し処理に渡します。")
                    md1, md2 = st.columns(2)
                    with md1:
                        if st.button("この内容で組み直し案を作る", key="mood_consult_apply", width="stretch"):
                            try:
                                request_text = consultation_value.strip() or "気分が変わったので、残り旅程を自由に組み直して。"
                                generate_execution_replan_preview(request_text, source_event="mood_change_consult")
                                st.session_state.show_mood_dialog = False
                                st.session_state.mood_flow_mode = "menu"
                                st.rerun()
                            except Exception as e:
                                ...
                                if st.session_state.debug_mode:
                                    st.exception(e)
                    with md2:
                        if st.button("戻る", key="mood_consult_back", width="stretch"):
                            st.session_state.mood_flow_mode = "menu"
                            st.rerun()
            ...
            if st.session_state.show_cancel_dialog:
                ...
                with cx1:
                    if st.button("簡易提案を出す", key="cancel_simple", width="stretch"):
                        try:
                            ...
                            st.error(f"イベント提案エラー: {e}")
                with cx2:
                    if st.button("自由に組み直し案を作る", key="cancel_replan", width="stretch"):
                        try:
                            ...
                            st.error(f"組み直し案生成エラー: {e}")
                with cx3:
                    if st.button("キャンセル", key="cancel_close", width="stretch"):
                        st.session_state.show_cancel_dialog = False
                        st.rerun()

            if st.session_state.replan_preview_draft:
                st.divider()
                preview_scope = st.session_state.get("replan_preview_scope", "all_future")
                scope_label = {"local": "直近の差分のみ", "rest_of_day": "その日の残りのみ", "all_future": "これから先すべて"}.get(preview_scope, "差分")
                st.info(f"自由入力をもとに、{scope_label}を見直した案です。気に入ればOKで実行シミュレーションへ反映できます。")
                ...
                with rp1:
                    if st.button("👌 OK（この案で反映）", key="apply_replan_preview", width="stretch"):
                        try:
                            ...
                            if st.session_state.debug_mode:
                                st.exception(e)
                with rp2:
                    if st.button("破棄する", key="discard_replan_preview", width="stretch"):
                        st.session_state.replan_preview_draft = None
                        st.session_state.replan_preview_request = ""
                        st.session_state.replan_preview_source = ""
                        st.session_state.replan_preview_scope = "all_future"
                ...
                if alternatives:
                    ...
                    for alt in alternatives:
                        with st.expander(f"案{alt.get('id', '?')}: {alt.get('title', '提案')}"):
                            ...
                            if st.button(f"この案を採用", key=f"apply_alt_{alt.get('id', 'x')}", width="stretch"):
                                try:
